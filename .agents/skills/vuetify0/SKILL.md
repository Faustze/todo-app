...
## Button Styling — todo-app pattern

The project uses a centralized SCSS mixin approach for button styling.
All button styles live in `src/assets/styles/_button.scss`:

```scss
// Available mixins:
@include button.btn-base;              // Base styles (layout, transitions, states)
@include button.btn-flat($color);      // Solid filled button
@include button.btn-outlined($color);  // Outlined button
@include button.btn-ghost($color);     // Transparent with colored text
@include button.btn-icon($color);      // Square icon button
```

`$color` is one of: `primary`, `secondary`, `error`, `warning`.

All colors come from `--v0-*` CSS custom properties set by `createThemePlugin`.
No hardcoded colors. Theme switch → all buttons recolor automatically.
...
.btn {
  ...
  &--flat--primary { @include button.btn-flat(primary); }
  &--outlined--primary { @include button.btn-outlined(primary); }
  // ... etc
}
</style>
```
...
**Do NOT:**
- Use inline `:style` for variant/color mapping — use SCSS classes
- Hardcode colors — always use `--v0-*` variables
- Duplicate button styles in component scoped blocks — import from `_button.scss`
